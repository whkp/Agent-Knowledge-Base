...
- 面向来源摄取、页面浏览、按命名策略的检索优先查询、状态、lint、回答反馈与显式可选综合的 MCP 工具。
...
评分是检索闭环唯一的评估信号，所以它能在工作台里读回来，而不是只能通过 API。点「没用」时可以指认具体哪条引用不对，回放因此能按页面级判断：
...
[`TODO.md`](TODO.md) 是工作清单，也是"接下来做什么"的唯一来源。当前头条三项：

1. 让评分能指明**是哪个引用不对**，使回放能按页面级而非集合级判断保留。
2. 解决图谱布局的 O(n²) 问题：实测 800 页时打开图谱要 1.8 秒。
3. 摄取改成带进度与内容哈希去重的后台队列；目前它会阻塞请求，且重复摄取未变化的来源会新增记录。
