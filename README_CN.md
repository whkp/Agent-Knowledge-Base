...
- 同一工作区上的四个画布视图：页面本身、由 `[[wikilinks]]` 派生的链接图谱、可为断链一键补页的健康检查，以及读回来的回答评分。
- 页面查询与原始资料查询都可选接入 OpenAI-compatible 模型综合，带引用依据，并在模型不可用时确定性回落。
- 页面查询会沿 `[[wikilinks]]` 从命中页面继续扩展——有把握走一跳，证据不足走两跳——并把扩展结果标记为 `related`。
...
- 每个回答下方可以点赞或贬低，贬低时可附原因，并可指认**是哪些引用不对**。反馈是 SQLite 里的业务数据，绝不写入 wiki。
- 来源可原地更新：同一来源的相同内容再次摄取会返回冲突，并指出该更新哪个 document；`PUT /api/knowledge-bases/{id}/documents/{document_id}` 就地替换正文，document id、raw 快照路径、来源页路径与主题页来源行都不变。内容相同但来源不同的两次摄取仍是**两份来源**，合并它们会丢掉溯源。
- 面向来源摄取与原地更新、页面浏览、按命名策略的检索优先查询、状态、lint、回答反馈与显式可选综合的 MCP 工具。
- 可选的 ChromaDB 与多语言 embedding 检索，同时服务于普通文档与可追溯的外部来源快照。

当前产品截图保存在 `demo/`，展示工作台、带引用的页面优先查询、派生链接图谱、反馈纸，以及可选的原始资料 RAG 模式。
...
评分是检索闭环唯一的评估信号，所以它能在工作台里读回来，而不是只能通过 API。点「没用」时可以指认具体哪条引用不对，回放因此能按页面级而不是集合级提问：

![AgentKB 反馈纸](demo/agentkb-feedback.png)
...
docs/          设计、检索与安装文档（每份都有中英两版）
skills/        通过 API 操作 Backend 的 Agent skill
proposals/     由记录下来的反馈生成、可评审的检索策略提案
AGENTS.md      面向贡献者与编码 Agent 的开发指南
TODO.md        工作清单：接下来做什么、为什么、怎么验证
```
...
| 计划 · 中文 | [TODO.md](TODO.md) | 接下来做什么、为什么、怎么验证，以及刻意不做的事 |
...
打开 `http://localhost:5173`，创建知识库、摄取一份来源、浏览生成的 Markdown 页面。画布可以在页面、派生链接图谱、健康检查与读回来的评分之间切换；阅读区始终显示当前页面的出链、反向链接和未解析链接。
...
重新采集一份来源不必产生第二个 document。同一来源的相同内容再次摄取会返回 `409 Conflict` 与已有 document id，而那个 document 可以就地替换——id、`raw/sources/` 快照路径、来源页与主题页来源行全部不变：

```bash
curl -X PUT http://localhost:8000/api/knowledge-bases/1/documents/7 \
  -H 'Content-Type: application/json' \
  -d '{"content": "<重新采集到的正文>", "source_captured_at": "2026-09-19T10:00:00Z"}'
```

没提交的字段保持原值，所以只改正文不会丢掉此前记录的溯源信息。
...
cd ../frontend
npm run build
npm run bench:layout          # 图谱布局耗时与质量指标
```
...
[`TODO.md`](TODO.md) 是工作清单，也是"接下来做什么"的唯一来源。当前头条：

1. 摄取改为带进度的后台队列。它仍阻塞请求，模型相关的主题维护也跑在这次请求里。
2. 内容级 lint。目前只查结构（断链、孤立页、缺摘要），不查页面到底说了什么。
3. 让回放覆盖 `mode="rag"` 的反馈，使原始资料路径与页面路径享受同一套对比。
4. 超过规模阈值后只画枢纽及其邻居。800 页图谱现在 315ms 能打开，但仍然是一团毛球。

该文件也记录了项目**刻意不做**的事及其原因，第一条就是"不在后端内置 Agent"。
