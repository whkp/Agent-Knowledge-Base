...
- 参与对比的评分样本：7 条（👍 4 / 👎 3）
- 建议：**auto** — 候选策略（deep、hybrid、planned）保留了全部 👍 引用，但没有隐藏任何默认策略仍会返回的、被指认过的无用引用，换上它们只是多付成本。
...
| `deep` | 14/14 | 0 | 0 | 0.29 | 2.0 |
| `hybrid` | 14/14 | 0 | 0 | 0.0 | 1.14 |
| `planned` | 14/14 | 0 | 0 | 0.0 | 1.14 |

## 逐条差异（相对当前默认策略）

### `local`

- 👍 `怎么分散风险` — 有用引用保留 2/4，直接 2 关联 0，跳数 1
